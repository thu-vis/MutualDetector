...
class FeatureDetect(Function):
    ...
    def forward(self, num_classes, bkg_label, top_k, conf_thresh, nms_thresh, loc_data, conf_data, prior_data):
        ...
        conf_preds = conf_data.view(num, num_priors,
                                    self.num_classes).transpose(2, 1)
        global_idxs = np.array(range(num_priors))
        global_idxs = torch.Tensor(global_idxs)
        ...
        for i in range(num):
            ...
            for cl in range(1, self.num_classes):
                c_mask = conf_scores[cl].gt(self.conf_thresh)
                scores = conf_scores[cl][c_mask]
                selected_idxs = global_idxs[c_mask]
                if scores.size(0) == 0:
                    continue
                # if scores.dim() == 0:
                #     continue
                ...
                output[i, cl, :count] = \
                    torch.cat((scores[ids[:count]].unsqueeze(1),
                               boxes[ids[:count]], 
                               selected_idxs[ids[:count]].unsqueeze(1)), 1)
